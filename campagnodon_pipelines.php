...
function _campagnodon_get_bank_transaction($id_transaction, $caller) {
	...
	if (!$transaction) {
		spip_log(
			__FUNCTION__.': Transaction introuvable pour id_transaction='.$id_transaction.'. Caller='.$caller,
	...
	if ($transaction['parrain'] !== 'campagnodon') {
		...
		spip_log(
			__FUNCTION__
				. ': programmer_sync_bank_result: cette ligne n\'a pas campagnodon comme parrain. Caller='
				. $caller,
...
function _campagnodon_get_abo_transactions($abo_uid, $caller) {
	...
	if (strncmp($abo_uid, 'uid:', 4) !== 0) {
		spip_log(
			__FUNCTION__
				. ': $abo_uid ne commence pas par uid:, je ne sais pas traiter. abo_uid="'.$abo_uid.'"Caller='.$caller,
			'campagnodon'._LOG_ERREUR
		);
		return [null, null];
	}
	...
	// Mais elle me permettra de remonter jusqu'à la transaction initiale
	// (et jusqu'à la transaction campagnodon initiale).
	...
	if (!$transaction_quelconque) {
		spip_log(
			__FUNCTION__
				.': Transaction campagnodon introuvable pour abo_uid='.$abo_uid.'. Caller='.$caller,
			'campagnodon'._LOG_DEBUG
		);
		return [null, null];
	}
	// 2022-10-24: je commente le code ci-dessous, et met le critère dans le sql_fetsel,
	// pour être compatible avec la migration depuis Souscriptions
	// if ($transaction_quelconque['parrain'] !== 'campagnodon') {
	// 	// Ça ne concerne pas notre plugin.
	// 	spip_log(
	//		__FUNCTION__.': cette ligne n\'a pas campagnodon comme parrain. Caller='.$caller, 'campagnodon'._LOG_DEBUG
	//	);
	// 	return [null, null];
	// }

	$campagnodon_transaction_quelconque = sql_fetsel(
		'*',
		'spip_campagnodon_transactions',
		'id_transaction=' . intval($transaction_quelconque['id_transaction'])
	);
	if (!$campagnodon_transaction_quelconque) {
		spip_log(
			__FUNCTION__
				. ': Transaction Campagnodon introuvable pour id_transaction='
				. $transaction_parent['id_transaction'].'. Caller='
				.$caller,
			'campagnodon'._LOG_ERREUR
		);
		return [null, null];
	...
	while (
		$campagnodon_transaction_quelconque
		&& ($campagnodon_transaction_quelconque['id_campagnodon_transaction_parent'] !== null)
	) {
		$id = $campagnodon_transaction_quelconque['id_campagnodon_transaction_parent'];
		$campagnodon_transaction_quelconque = sql_fetsel(
			'*',
			'spip_campagnodon_transactions',
			'id_campagnodon_transaction=' . intval($id)
		);
		if (!$campagnodon_transaction_quelconque) {
			spip_log(
				__FUNCTION__.': Transaction Campagnodon introuvable id_campagnodon_transaction='.$id,
				'campagnodon'._LOG_ERREUR
			);
			return [null, null];
		}
	...
	if ($transaction_quelconque['tracking_id'] == $campagnodon_transaction['id_campagnodon_transaction']) {
		...
		$transaction = sql_fetsel(
			...
			[
				'parrain=' . sql_quote('campagnodon'),
				'tracking_id='.sql_quote($campagnodon_transaction['id_campagnodon_transaction'])
			],
			'',
			'id_transaction ASC'
		);
		if (!$transaction) {
			spip_log(
				__FUNCTION__
					. ': Transaction introuvable pour parrain=campagnodon & tracking_id='
					. $campagnodon_transaction['id_campagnodon_transaction']
					. '. Caller='.$caller,
				'campagnodon'._LOG_ERREUR
			);
			return [null, null];
...
function campagnodon_bank_dsp2_renseigner_facturation($flux) {
	...
	if (!(
		...
		and	$campagnodon_transaction = sql_fetsel(
			'*',
			'spip_campagnodon_transactions',
		...
		spip_log(
			'Pipeline bank_dsp2_renseigner_facturation: Impossible de trouver la transaction campagnodon.',
			'campagnodon'._LOG_ERREUR
		);
		return $flux;
	}
	...
	$dsp2_renseigner_facturation = campagnodon_fonction_connecteur(
		$campagnodon_transaction['mode'],
		'dsp2_renseigner_facturation'
	);
	if (!$dsp2_renseigner_facturation) {
		spip_log(
			'Pipeline bank_dsp2_renseigner_facturation: connecteur dsp2_renseigner_facturation non trouvé pour le mode '
				. $campagnodon_transaction['mode'].'.',
			'campagnodon'._LOG_ERREUR
		);
		return $flux;
	...
	spip_log(
		'Pipeline bank_dsp2_renseigner_facturation: Connecteur dsp2_renseigner_facturation trouvé.',
		'campagnodon'._LOG_DEBUG
	);
	...
	if (!$data) {
		spip_log(
			'Pipeline bank_dsp2_renseigner_facturation: Impossible de trouver les informations à fournir.',
			'campagnodon'._LOG_ERREUR
		);
		return $flux;
	}
...
function programmer_sync_bank_result($flux) {
	...
	if (!empty($flux['args']['row']) && !empty($flux['args']['row']['id_transaction'])) {
		...
		if ($transaction['parrain'] !== 'campagnodon') {
			// Ça ne concerne pas notre plugin.
			spip_log(
				'programmer_sync_bank_result: cette ligne n\'a pas campagnodon comme parrain, je saute.',
				'campagnodon'._LOG_DEBUG
			);
			return;
		}
		...
		spip_log(
			'programmer_sync_bank_result: la ligne de donnée n\'est pas dans le flux, je dois chercher en base.',
			'campagnodon'._LOG_DEBUG
		);
		$id_transaction = $flux['args']['id_transaction'];
	...
	if (!$id_campagnodon_transaction) {
		spip_log(
			'Transaction introuvable pour id_transaction='.$id_transaction.' car pas de tracking_id.',
			'campagnodon'._LOG_ERREUR
		);
		return;
	}
	spip_log(
		'programmer_sync_bank_result: je dois programmer une synchronisation de la ligne id_campagnodon_transaction='
...
function campagnodon_bank_abos_decrire_echeance($flux) {
	...
	spip_log(
		'campagnodon_bank_abos_decrire_echeance: voici les infos remontées: '.print_r($flux['data'], true),
		'campagnodon'._LOG_DEBUG
	);
	return $flux;
}
...
function campagnodon_bank_abos_activer_abonnement($flux) {
	if ($flux['data']) {
		spip_log(
			__FUNCTION__.': il y a déjà quelque chose dans data, un autre plugin a dû répondre.',
			'campagnodon'._LOG_DEBUG
		);
		return $flux;
	}
	...
	$id_campagnodon_transaction = $transaction['tracking_id'];
	spip_log(
		__FUNCTION__.': campagnodon doit gérer la transaction='
			. $id_transaction.', transaction campagnodon='
			. $id_campagnodon_transaction,
		'campagnodon'._LOG_DEBUG
	);
	...
	if (strncmp($transaction['statut'], 'ok', 2) == 0) {
		spip_log(
			__FUNCTION__.': la transaction campagnodon '.$id_campagnodon_transaction.' est en statut ok, j\'active',
			'campagnodon'._LOG_DEBUG
		);
		$statut_recurrence = 'encours';
	} else {
		spip_log(
			__FUNCTION__.': la transaction campagnodon '
				. $id_campagnodon_transaction.' n\'est pas encore en statut ok, je n\'active pas encore',
			'campagnodon'._LOG_DEBUG
		);
	...
	spip_log(
		__FUNCTION__.': Je passe le statut de récurrence à '
			. $statut_recurrence.' pour la transaction campagnodon '.$id_campagnodon_transaction,
		'campagnodon'._LOG_DEBUG
	);
	if (false === sql_updateq(
		...
		spip_log(
			__FUNCTION__.': Erreur à la modification de la transaction campagnodon '.$id_campagnodon_transaction,
			'campagnodon'._LOG_ERREUR
		);
		return $flux;
	}
	...
	// SPIP Bank ne semble pas utiliser la valeur de retour, mais on va quand même mettre quelquechose.
	$flux['data'] = $id_campagnodon_transaction;
	return $flux;
}
...
function campagnodon_bank_abos_preparer_echeance($flux) {
	...
	list (
		$transaction_parent,
		$campagnodon_transaction_parent
	) = _campagnodon_get_abo_transactions($abo_uid, __FUNCTION__);
  ...
  $id_transaction = $inserer_transaction($montant_total, $transaction_options);
	...
	$transaction_idx_distant = get_transaction_idx_distant(
		$mode_options,
		$id_campagnodon_transaction,
...
function campagnodon_bank_abos_renouveler_abonnement($flux) {
	...
	$id_campagnodon_transaction = $transaction['tracking_id'];
	spip_log(
		__FUNCTION__.': campagnodon doit gérer id_campagnodon_transaction='.$id_campagnodon_transaction,
		'campagnodon'._LOG_DEBUG
	);
...
function campagnodon_bank_abos_resilier($flux) {
	...
	if (
		...
		and $mode_paiement = sql_getfetsel(
			'mode',
			'spip_transactions',
			'abo_uid=' . sql_quote($abo_uid, '', 'text'),
			'',
			'id_transaction DESC'
		)
	) {
	...
	spip_log(
		__FUNCTION__.': Je passe le statut de récurrence à '
			. $statut_recurrence.' pour la transaction campagnodon ' . $id_campagnodon_transaction_parent,
		'campagnodon'._LOG_DEBUG
	);
	if (false === sql_updateq(
		'spip_campagnodon_transactions',
		...
		spip_log(
			__FUNCTION__.': Erreur à la modification de la transaction campagnodon '.$id_campagnodon_transaction_parent,
			'campagnodon'._LOG_ERREUR
		);
		return $flux;
	}
